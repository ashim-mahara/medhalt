...
class FullDataEval(object):
    ...
    def __init__(self, folder_name, correct_score=1, incorrect_score=-0.25):
        self.evaluations = []
        self.all_files = {k.split('.json')[-2].split('/')[-1]: k for k in glob.glob(f'./{folder_name}/*json')}
    ...
    def handle_exceptions(self, task_name, sample, exception):
        print(task_name, sample['id'], sample, exception)
        return 1
            ...
            try:
                ...
                for key in possible_keys:
                    if key in sample['output']:
                        predicted_answer = sample['output'][key]
                        break

                if predicted_answer is None:
                    raise KeyError("No valid key found in 'output'")

                if self.evaluate_answer(str(predicted_answer),sample['correct_answer']):
                    correct += 1
                else:
                    wrong += 1
    ...
    def IR_pmid2title_pubmedlink2title_eval(self, data, task_name):
        ...
        for sample in tqdm(all_files_data):
            try:
                predicted_title = sample['output']['paper_title']

                if self.evaluate_answer(predicted_title, sample['Title']):
                    correct += 1
                else:
                    wrong += 1
            except Exception as e:
    ...
    def IR_title2pubmedlink_abstract2pubmedlink_eval(self, data, task_name):
        ...
        for sample in tqdm(all_files_data):
            try:
                predicted_url = sample['output']['url']

                if self.evaluate_answer(predicted_url, sample['url']):
                    correct += 1
                else:
                    wrong += 1
            except Exception as e:
    ...
    def reasoning_nota_eval(self, data, task_name):
        ...
        for sample in tqdm(all_files_data):
            try:
                predicted_answer = str(sample['output']['cop'])

                if self.evaluate_answer(predicted_answer, sample['correct_answer']):
                    correct += 1
                else:
                    wrong += 1
            except Exception as e:
    ...
    def reasoning_fake_eval(self, data, task_name):
        ...
        for sample in tqdm(all_files_data):
            try:
                predicted_answer = str(sample['output']['cop']).lower()
    ...
    def run_all_evaluations(self):
        ...
        print("self.all_files.keys()", self.all_files.keys())

        # import sys
        # sys.exit()
        for key in self.all_files:
            try:
                if "gen_kwargs" in key:
                    continue
